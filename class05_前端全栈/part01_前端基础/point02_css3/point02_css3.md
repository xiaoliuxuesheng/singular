...
- 浏览器默认的排版方式就是标准流，也称普通流、文档流；实际上就是一个网页内标签元素正常从上到下，从左到右排列顺序的意思

- 标准流排版特点：标准流中有两种排版方式：水平排版（行内元素）、垂直排版（块级元素）
  - 水平排版：如果元素是行内或行内块元素会水平排版
  - 垂直排版：如果元素是块级元素会垂直排版；
...
1. 浮动流是一种“半脱离标准流”的排版方式，浮动后会移动到父元素中指定的位置；
...
3. 浮动元素是不可以设置`margin 0 auto`的；
4. 浮动元素是不能撑起父元素的高度；
5. 浮动流是不区分行内元素、块级元素或行内块元素：浮动所有元素都可以设置宽和高；

### <font color=blue size=4>▲ 版心和布局流程</font>
