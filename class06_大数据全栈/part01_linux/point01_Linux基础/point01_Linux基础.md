...
1. **Unix的发展历程**
   ...
   | 1973年 | 用C语言重写了UNIX：UNIX现在主要发行版本 AIX    →    IBM UC     →    HP Solaris    →    SUN Linux    →    Read Hat Linux、Ubuntu、FreeBSD |
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;VMware虚拟机安装：是一个虚拟的PC的软件，可以在现有操作系统（宿主机）上模拟出一个新的硬件环境（相当于模拟了一台新的电脑），可以在这个模拟的硬件环境中安装操作系统，与宿主机是两个完全独立的操作系统；32位操作系统只能识别到3.1G的内存；
- **步骤一**：稍后安装操作系统
- **步骤二**：内存设置：Linux最小内存628M（小于会自动开启简易安装），运行时内存只用到几十兆；建议设置1024M
...
1. **磁盘分区**：使用分区编辑器在磁盘上划分几个逻辑部分（不是吧磁盘隔开成小物理单元块），磁盘分区后，不同类的目录与文件可以存储进不同的分区（类似Windows系统中的盘）；
2. **分区表以及分区形式**
   - **MBR（Master Boot Record）- 主引导记录分区表**：古老的分区表，最大支持2.1T硬盘（超出的部分不能使用）；每块硬盘最多支持4个分区；可以将其中一个主分区配置为扩展分区，通过将扩展分区拆分为逻辑分区（最多拆分为10个），就突破4个分区的限制；
     - 主分区：最多只有4个（操作系统必须要有一个主分区，用于操作系统的激活盘）
     - 扩展分区：
       - 每块硬盘的扩展分区最多只能有1个：即主分区加扩展分区最多有4个
       - 扩展分区不能写入数据，主要用户拆分逻辑分区
     - 逻辑分区：可以存储数据
   - **GPT（Generative Pre-Training）- 全局唯一标识分区表**：GPT支持9.4ZB硬盘，理论上支持的分区数没有限制（Windows限制128个）
3. **格式化**：分区后必须将分区格式化（高级格式化）：又称逻辑格式化，格式化的目的是写入文件系统（FAT16、FAT32、NTFS、EXT2、EXT3、EXT4、XFS等）越先进的文件系统支持越大的分区，支持越大的单个文件读写越快、越安全；文件系统的主要作用是在磁盘的特定区域写入特定数据：如在分区中划出一片用于存放文件分配表、目录表等用于文件管理的磁盘空间（简述就是为了写入文件系统）；例如：Linux的EXT4格式化原理：分区分为两个部分，小的部分用于保存文件的iNode（128字节）索引：包含文件权限、node号、修改时间，大的部分分为4kb的block块存储数据（Windows磁盘碎片整理就是将文件的block连续存放）；
   - Windows系统大部分是NTFS；
   - REHL6中用的EXT4；
   - CentOS7中用XFS；
4. **硬件设备文件名**：Linux中所有硬件设备是以文件形式表现的
...
5. 设备挂载：（Windows中定义盘符）是指Linux中的**已存在空目录**挂载到硬盘设备上的步骤
   - 必须分区
...
1. 命令提示符
   ...
   > - localhost：登录的主机IP，hostname可以查看
   > - ~：当前所在目录的最后一级目录
...
2. 命令格式
   ...
   > - `[]`：方括号代表里面的值是可选项
   > - 选项的作用：用于调整命令的功能
   > - 参数的作用：用于指定命令的执行对象，如果参数缺失表示命令有默认对象
...
- 长格式文件列表字段列说明
  ...
  - 第一列：权限
  - 第二列：引用计数；①文件的引用计数是该文件的硬链接个数、②目录的引用技术代表该目录包含多少个一级子目录（包括`./`和`../`）
  - 第三列：所有者
  - 第四列：所属组
  - 第五列：文件大小，默认单位是字节，-h选项使用易读的方式显示文件大小
  - 第六列：文件修改时间；
  - 第七列：文件名称
...
- more的使用技巧，配合cat可以分页查看并显示行号

  ```shell
   cat -n anaconda-ks.cfg |more^C
  ```
...
> - 选项`-n`：默认只查看10行，该选项用于指定查看的行数，n是可以省略的，例如直接使用-20表示显示前20行
...
- 链接介绍：在文件直接建立链接，在Linux分区两部分：①node节点②block文件快；并且Linux的每个文件都有自己的I节点和block，在文件的I节点保存的是该文件的节点号、时间、权限、保存位置，在文件的block块保存的文件内容；在目录的block块保存的是该目录下所有文件的文件名称，root目录也有自己的I节点和block块，根目录的I节点号固定是2

- 创建命令格式：硬链接，不建议使用，硬链接标记不清，很难确认链接文件位置
...
- 创建命令格式：软链接
  ...
  > 1. 软链接和源文件拥有不同的INode和Block，软链接只保存源文件的INode
  > 2. 两个文件修改任意一个，另一个都会改变
  > 3. 删除软链接，源文件可以使用；删除源文件，软链接不能使用；软链接的最终访问权限需要参考源文件权限

## 2.2 权限命令

### 1. 基本权限

- 使用`ls -l`查看长格式文件列表，在文件列表中会显示文件权限列：共十一个字符

  ```shell
  [root@BigDataNode104 ~]# ll
  总用量 4
  -rw-r--r--. 2 root root 17 8月   6 09:48 link.txt
  ```
...
- 权限列十字符含义说明
  ...
  | 列       | 作用                     | 说明                                                         |
  | -------- | ------------------------ | ------------------------------------------------------------ |
  | 第1列    | 文件类型                 | `-`：文件<br />`d`：目录<br />`l`：软链接<br />`p`：管道符文件(设备文件)<br />`c`：字符设备文件<br />`b`：块设备文件 |
  | 第2~4列  | u:文件所有者权限         | `r`：read,读取的权限<br />`w`：write,写权限<br />`x`：execute,可执行权限<br />`-`: 表示对应位置没有权限 |
  | 第5~7列  | g:文件所属组其他用户权限 | `r`：read,读取的权限<br />`w`：write,写权限<br />`x`：execute,可执行权限<br />`-`: 表示对应位置没有权限 |
  | 第8~10列 | o:其他人权限             | `r`：read,读取的权限<br />`w`：write,写权限<br />`x`：execute,可执行权限<br />`-`: 表示对应位置没有权限 |
  | 第11列   | 无特殊含义               |                                                              |

- 权限作用：文件权限的读、写、执行对文件和目录的作用是不同的

  | 文件类型 | 读-r             | 写-w             | x-执行       |
  | -------- | ---------------- | ---------------- | ------------ |
  | 文件     | 读取文件中内容   | 修改文件中的数据 | 可运行       |
  | 目录     | 查看目录下子文件 | 修改目录中的文件 | 可以进入目录 |
...
- 修改权限方式一：给指定组(u、g、o)添加指定权限,+表示添加,-表示去除
...
- 修改权限方式四：数字权限，权限分三组（用户、组、其他），每组权限都是r、w、x，每个权限代表一个数字，一种权限表示数字之和，修改权限时候只需要指定三个数字代表三组的权限

  > 对目录的权限其实只有是三个:0=无权限;5=可进入浏览;7=可进入操作子文件
...
1. chown注意事项
   ...
   - 普通用户不能修改文件的所有者,即使文件属于这个普通用户的,只有超级用户修改文件的所有者

2. chown命令格式

   ```shell
   chown [选项] 所属用户:所属组 文件\目录名称
   ```
...
> | 选项   | 说明                                                         |
> | ------ | ------------------------------------------------------------ |
> | -E     | 等于egrep命令,支持扩展正则表达式                             |
> | -C数值 | context:表示匹配行的上下                                     |
> | -c     | 统计出现次数,类似 命令`wc -l`                                |
> | -A数值 | after: 匹配到的内容并且向后显示的行数                        |
> | -B数值 | before: 匹配到的内容并且向前显示的行数                       |
> | -v     | 取反:表示不显示查找到的,比如使用ps时候过滤会把自己也过滤出来 |
> | -n     | 显示行号                                                     |
> | -i     | 忽略大小写                                                   |
> | -w     | 精确匹配                                                     |
...
1. sed概述：stream edit表示流式编译器。是利用脚本来处理文本文件，sed 可依照脚本的指令来处理、编辑文本文件。**运行模式**：当处理数据时，Sed 从输入源一次读入一行，并将它保存到所谓的模式空间pattern space中。所有 Sed 的变换都发生在模式空间。变换都是由命令行上或外部 Sed 脚本文件提供的单字母命令来描述的。大多数 Sed 命令都可以由一个地址或一个地址范围作为前导来限制它们的作用范围。

   ![image-20220806181914121](https://raw.githubusercontent.com/xiaoliuxuesheng/resource_image/master/img/image-20220806181914121.png)
...
2. sed格式
   ...
   > | 选项参数 | 功能                                                         |
   > | -------- | ------------------------------------------------------------ |
   > | -n       | 屏蔽默认输出功能，默认set会把匹配到的数据显示到屏幕上,常与命令p一起用 |
   > | -r       | 支持扩展正则                                                 |
   > | -i[后缀] | 修改源文件:将修改结果写入文件,否则修改的是内存数据 - 如果设置了后缀,sed会将数据备份 |
   > | -f       | 指定需要执行的脚本文件,需要提前将sed指令写入文件中           |
   >
   > **内置命令字符**:用于对文件进行增删改查的操作
   >
   > | 命令字符          | 说明                                           |
   > | ----------------- | ---------------------------------------------- |
   > | i 文本            | 在匹配的数据行前插入文本内容                   |
   > | a 文本            | 在匹配的数据行后追加文本内容                   |
   > | d                 | 删除匹配的数据行整行内容(行删除)               |
   > | c 文本            | 将匹配的数据行整行内容替换为特定的文本内容     |
   > | s/regexp/replace/ | 使用正则匹配，将匹配的数据替换为特定的内容     |
   > | p                 | 打印当前匹配的数据行                           |
   > | l                 | 打印当前匹配的数据行(显示控制字符，如回车符等) |
   > | =                 | 打印当前读取的数据所在的行数                   |
   > | r 文件名称        | 从文件中读取数据并追加到匹配的数据行后面       |
   > | w 文件名称        | 将当前匹配到的数据写入特定的文件中             |
...
3. 案例
   ...
   - 输出文件指定行号的内容
     ...
     ```shell
     # p命令:发现打印结果将没一行都打印的两次,因为默认输出会将未匹配的也返回都结果,
     # p命令匹配了所有行,所以每一行都打印的两次,使用-n选项配合p命令打印匹配到的行
     sed 'p' a02.txt 
     sed -n 'p' a02.txt 
   ...
   - 插入命令
     ...
     # 使用命令a在匹配文本后追加
     sed /男/aadd-to-file a02.txt 
     
     # 使用-i参数修改文件
     sed /男/aadd-to-file a02.txt -i
     ```
   ...
   - 替换命令
     ...
     # 使用s命令替换匹配字符,g表示全局替换
     sed s/男/未知/ a02.txt 
     sed s/男/未知/g a02.txt 
     ```

   - 使用-e选项sed操作多次

     ```shell
     sed -e s/男/未知/ -e 1p -n a02.txt
     ```
...
1. awk概述:有强大的文本格式化的能力,awk更是一门编程语言,支持判断、数组、循环等功能；awk默认一空格为分隔符（多个空格也会识别为一个空格作为分隔符），按行处理文件，一行处理完毕后再处理下一行，根据用户指定的分隔符工作，如果没有指定则使用默认的分隔符；

2. awk语法:awk语法由一系列条件和动作组成，在花括号内可以有多个动作，多个动作之间用分号分隔，在多个条件和动作之间可以有若干空格，也可以没有。awk最常用的action是print,作用是将有一系列内置变量和自定义变量通过相关的分支循环语句进行输出
...
3. awk使用案例
   ...
   - awk参数说明
     ...
     ```shell
     # 默认分隔符是空格,输出第一列会把整行都输出
     [root@BigDataNode104 Documents]# awk "{print $1}" a02.txt 
     101,张三,男,ceo,3200
     201,李思,女,cfo,3300
     103,吴广,男,emp,2000
     104,王五,男,emp,1000
     ...
     # 指定分隔符
     [root@BigDataNode104 Documents]# awk -F, '{print $1}' a02.txt 
     101
     201
     103
     104
   ...
   - 输出内置变量中指定的内容
     ...
     ```shell
     # 输出每一行
     [root@BigDataNode104 Documents]# awk '{print $0}' a02.txt 
     101,张三,男,ceo,3200
     201,李思,女,cfo,3300
     103,吴广,男,emp,2000
     104,王五,男,emp,1000
     ...
     # 指定分隔符,输出第多列(列之间用逗号分隔,输出时候会根据默认的输出格式)
     [root@BigDataNode104 Documents]# awk -F, '{print $1,$3,$5}' a02.txt 
     101 男 3200
     201 女 3300
     103 男 2000
     104 男 1000
     ...
     # 自定义输出内容,按需要的内容进行列内容的拼接
     [root@BigDataNode104 Documents]# awk -F, '{print "id=",$1,",性别=",$3,",工资=",$5}' a02.txt 
     id= 101 ,性别= 男 ,工资= 3200
     id= 201 ,性别= 女 ,工资= 3300
     id= 103 ,性别= 男 ,工资= 2000
     id= 104 ,性别= 男 ,工资= 1000
   ...
   - 显示文件第N行
     ...
     ```shell
     # 输出指定行,指定NR的值
     [root@BigDataNode104 Documents]# awk -F, 'NR==2,NR==3{print $0}' a02.txt 
     201,李思,女,cfo,3300
     103,吴广,男,emp,2000
     
     # 使用NR给输出添加行号
     [root@BigDataNode104 Documents]# awk '{print NR,$0}' a02.txt 
     1 101,张三,男,ceo,3200
     2 201,李思,女,cfo,3300
     3 103,吴广,男,emp,2000
     4 104,王五,男,emp,1000
     5 105,孙悟空,男,emp,200
     6 106,林志玲,女,emp,3000
     
     # 使用NF输出需要的列
     [root@BigDataNode104 Documents]# awk -F, '{print $1,$(NF-1)}' a02.txt 
     101 ceo
     201 cfo
     103 emp
     104 emp
   ...
   - awk变量
     ...
     ```shell
     # -v修改输出分隔符变量值
     [root@BigDataNode104 Documents]# awk -F, -v "OFS=\t" '{print $1,$3,$5}' a02.txt
     101     男      3200
     201     女      3300
     103     男      2000
     104     男      1000
     ...
     # 使用内置变量值
     [root@BigDataNode104 Documents]# awk -F, '{print NR,NF,$0}' a02.txt 
     1 5 101,张三,男,ceo,3200
     2 5 201,李思,女,cfo,3300
     3 5 103,吴广,男,emp,2000
     4 5 104,王五,男,emp,1000
     5 5 105,孙悟空,男,emp,200
     6 5 106,林志玲,女,emp,3000
     
     # 把逗号当做行分隔符
     [root@BigDataNode104 Documents]# awk -F, -v "RS=," '{print $0}' a02.txt 
     101
     张三
     男
     ceo
   ...
   - awk的if语句:最外面的花括号属于组合语句动作系列，而里面的花括号可以看作是条件判断语句的一部分。

     ```shell
     # 判断列的值,决定是否输出
     [root@BigDataNode104 Documents]# awk -F, '{if($1=="105") {print $0}}' a02.txt 
     105,孙悟空,男,emp,200
     
     # 使用多分支
     [root@BigDataNode104 Documents]# awk -F, '{if($1=="105") {print $0}else{print $1}}' a02.txt 
     101
     201
     103
     104
   ...
   - for循环
     ...
     ```shell
     # 把105行输出三次
     [root@BigDataNode104 Documents]# awk -F, 'BEFIN{count=1} {for(count=1;count<4;count++)if($1=="105"){print$0}}' a02.txt 
     105,孙悟空,男,emp,200
     105,孙悟空,男,emp,200
     105,孙悟空,男,emp,200
     ```
   ...
   - while循环
     ...
     ```shell
     [root@BigDataNode104 Documents]# awk -F, 'BEGIN{count=0}{while(count<4){print $0;count++} count=0}' a02.txt 
     101,张三,男,ceo,3200
     101,张三,男,ceo,3200
     101,张三,男,ceo,3200
     101,张三,男,ceo,3200
