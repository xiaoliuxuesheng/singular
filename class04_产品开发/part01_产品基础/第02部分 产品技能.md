...
​		需求与产品产品简单描述就是：需求是用户所欲需要的，产品是满足用户需求的东西；

​		所以产品经理需要解决的问题就是：① 确定用户真实需要的；② 如何满足用户需求；③ 如何更好的满足用户需求（用户还有没有其他需求）。因此，**产品经理对于用户需求的理解和把握**，是最重要的核心能力之一。

​		需求分析本质是对用户需求的更深层次的思考：

​				① 根据场景：时间 + 地点 + 人物 + 环境 + 事件

​				② 根据人性：马斯洛需求、逐利、两性、懒惰、虚荣、同理心、安全感、社交货币等等。。。

## 3.2 道斯矩阵

### <font color=red>**1.  SWOP概述**</font>

​		SWOT分析法也叫态势分析法，是一种常用的战略规划分析方法，代表分析企业的优势（strengths）、劣势（weakness）、机会（opportunity）和威胁（threats），企业界使用它作为战略规划工具，SWOT分析通常在创新流程的早期执行，<u>初衷在于帮助企业在商业环境中找到自身定位</u>，并在此基础上作出决策，现在得到了更广泛的使用。
...
​		**被称为思考的梯子**：需要由浅入深，深入思考后才能发现本质；把用户需求转换为产品功能的过程时，最重要的一步就是将用户需求转为为产品需求，如果转换过程发现是一个伪需求，就不需要为产品添加功能。

​		用户需求转换为产品需求的过程中，3+1本质是结合产品能力不断的给用户需求提出问题与对应的解决方案，在一系列的自问自答中逐步确定用户的真实需求和产品功能；

​		经典问题：（**本质是添加产品需求后的数据有什么变化？**）
...
### <font color=red>**1.  KANO概述**</font>

​		优秀的产品有一个更重要的特点，那就是**优秀的产品总是给用户带来惊喜**；东京理工大学教授狩野纪昭(Noriaki Kano)1979年在一篇论文中提出了KANO模型这个用于评估需求的经典模型。

​		KANO是通过对产品功能完备程度和用户满意度两个维度进行分析用户需求的一种工具，用户将用户需求进行优先级排序；

​		<img src='./resources/part02_11_kano模型'/>

1. **基本型需求**是顾客认为产品“必须有”的属性或功能。
2. **期望型需求**要求提供的产品或服务比较优秀，但并不是“必须”的产品属性或服务行为有些期望型需求连顾客都不太清楚，但是是他们希望得到的。
3. **兴奋型需求**要求提供给顾客一些完全出乎意料的产品属性或服务行为，使顾客产生惊喜。当产品提供了这类需求中的服务时，顾客就会对产品非常满意，从而提高顾客的忠诚度。

### <font color=red>**2. 需求排序**</font>

<img src='./resources/part02_12_kano优先级'/>

## 3.5 四象限法则 

​		需求的优先级是个时间管理的问题。如果我们有充裕的资源，无限的时间，那么只要确认可以接的需求，我们都可以做。然而往往现状是：没有资源，没有时间，N个需求压过来，每个都说自己最重要，每个都希望今天提需求明天就上线，这时候，就必须评估需求的优先级。

​		《麦肯锡思维》中提到，根据事件的重要性和紧迫性为坐标轴，可以将所有事情区分放在四个象限：重要且紧急、重要不紧急、不重要但紧急、不重要不紧急。

<img src='./resources/part02_12_四象限'/>

## 3.6 问卷调查

## 3.7 用户角色

​		是从用户群体中抽象而成的典型用户，主要包含用户描述用户相关属性，这些属性经过数据精炼后成为用户标签。

​		是在产品研发早期，需要借助用户角色帮助产品经理的决策与设计，理解用户的真实需求。

## 3.8 用户标与签用户画像

​		用户标签，是一个非常基础的，估计人人都听过的，但是却经常弄混淆的概念，标签是由原始数据，经过整理、加工、分类所得的一个抽象的符号。它是用户画像、精准营销、个性推荐、智能投放等等各种系统的砖石

​		使用用户标签，就能快速、方便地细分用户群体，锁定更有需求的人，实现更精准的营销/服务。是在应用上线后根据数据（10万起步）进行分析并高度精炼后的特征标识用于描述某一个群体的用户，由这些标签描述的这一类用户成为这类用户的用户画像

[在线制作用户画像工具]: http://yciyun.com

## 3.9 用户群体卡片

​		单一用户画像无法全面评估真实需求，由多个用户画像组成用户群体卡片
