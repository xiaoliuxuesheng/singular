...
- 使用案例：不足之处，使用RSA加签验签都需要公钥和私钥；加签和验签应该只需要一个秘钥就可以了；

  ```java
  /**
  1. JWT token对象：create方法创建token
  2. Algorithm 签名算法
  3. JWTVerifier：验签对象
  */
  @Test
  public void testHmac() throws Exception {
      ...
      .withNotBefore(new Date(System.currentTimeMillis() + 100))
  ...
  public void testHmac() throws Exception {
      ...
      String sign = JWT.create().sign(algorithm);
  }
...
- Maven：只有一个编译时依赖项，其余的声明为运行时依赖项。这是因为JJWT在应用程序中明确设计的API，使其所有其他内部实现细节降级为仅运行时依赖项。永远不要使用编译范围将`jjwt-impl`添加到项目中，始终使用运行时范围声明它。精心策划`jjwt-api`并确保它包含的内容并尽可能保持向后兼容。运行时`jjwt-impl`策略为JJWT开发人员提供了随时随地更改内部包和实现的灵活性。
...
- 使用案例：提供了完整的JWT实现，库本身自带有RSA的实现，对称加密与非对称加密都有提供实现
