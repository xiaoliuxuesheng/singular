...
​		工厂模式是使用是非常灵活的，工厂类创建的对象类型可以不做严格区分，即理论上一个工厂类可以创建出任何类型的对象，Spring的IOC容器也是使用工程方法模式，管理着Spring应用中的所有对象。

​		工厂方法模式的核心思路：需要定义一个工厂类，在这个工厂类的方法中完成对象的创建，每次需要创建对象时候就调用工厂类中的方法即可；

​		在Java代码中方法有实例方法和静态方法，如果工厂类中创建对象的方法是实例方法，则工厂方法模式也称为简单工厂模式。如果工厂类中创建对象的方法是静态方法，则工厂方法模式也被称为静态工厂方法模式；

<font size=4 color=blue>1. 简单工厂模式</font>：简单工厂的核心对方法的设计

- 在工厂类中添加多个实例方法，每个实例方法创建一种类型的对象
- 在工厂类中设计一个方法，根据方法参数标识对象，用代码逻辑判断所需要创建的对象类型
- 在工厂类中设计一个方法，方法参数的所需要创建对象类的全限定名称，利用Java反射原理创建对象

<font size=4 color=blue>1. 静态工厂方法模式</font>：和实例方法设计方法相同，唯一区别是方法属于静态方法，可以使用类名直接执行。
