...
1. 线程局部变量
   ...
   - 简介:THreadLocal提供线程局部变量,这些变量与正常变量不同,每个线程在访问ThreadLocal的时候,都有自己独立的变量副本,ThreadLocal通常是类中的私有静态字段,作用的将状态(线程数据)与线程关联起来

     > 实现了每个线程都有自己专属的本地变量副本(每个线程都用自己的数据,不和其他线程分享),解决线程安全问题
