...
1. 节点：根节点、子节点、兄弟节点（是否同意父节点）、叶子节点
2. 度
   - 节点的度：当前节点子树的个数；（叶子节点的度为0）
   - 树的度：树中所有子节点中最大的度；
3. 深度
   - 节点的深度：从根节点到当前节点的唯一路径上的节点的总数；
   - 树的深度：所有节点中最大的深度；
4. 高度
   - 节点的高度：从当前节点到最远叶子节点的唯一路径上节点的总数；
   - 树的高度：所有节点中最大的高度；
5. 有序树、无序树、森林
...
- 叶子节点只会出现在最后2层，并且最后一层的叶子节点都靠左对齐；
...
1. 树的常用术语
   ...
   - 森林：多颗子树构成森林
2. 树的分类
   - 二叉树：每个节点最多只有两个子节点的二叉树，子节点分为：左节点和右节点
     - 完全二叉树:二叉树的所有叶子节点都在最后一层或者倒数第二层,并且最后一层的叶子节点在左边练习,倒数第二次的叶子节点在右边连续
     - 满二叉树：二叉树所有的叶子节点都在最后一层，并且节点总数=2^n-1
     - 二叉搜索树
     - 平衡二叉树  红黑树
     - 堆树
   - B树
