...
<img src='./imgs/01_基本概念'/>

### :dash: producer

- **生产者**表示数据的发布者，该角色将消息发布到Kafka的topic中。

- Kafka即broker接收到生产者发送的消息后，broker将该消息**追加**到当前用于追加数据的segment文件中。
...
- 在Kafka中使用一个类别属性来划分数据的所属类，划分数据的所属类称为topic，如果把Kafka作为一个存储数据的数据库，topic可以看做是数据中的一张表，表名就是topic的名称；
- 物理上不同Topic的消息分开存储；
...
### :dash: partition offset

- 每条消息都有一个当前partition下唯一的64字节的offset，他指明了这条消息的起始位置；

### :dash: replicas of partition

- 副本是一个分区的备份，其中有且只有一个Leader，Leader负责当前数据读写的partition；
...
- ZooKeeper负责维护和协调broker，当kafka系统中新增broker或某个broker发生故障时候，有ZooKeeper通知生产者和消费者，生产者和消费者根据ZooKeeper和broker状态协调消息的发布和订阅；

### :dash:  consumer Group

- 每个Consumer属于一个特定的Consumer Group（可为每个Consumer指定group name，若不指定group name则属于默认的group）。

### :dash: AR（Assigned Replicas）

- 分区中所有副本统称为AR

### :dash: ISR（In-Sync Replicas）

- Leader与其保持一致的副本组成ISR

### :dash: ​ OSR（Out of Sync Replicas）

- 与Leader副本同步滞后过多的副本

 ### :dash: HW​ （High Watermark）

- 高水位：标识了一个特定的offset，消费者只能拉取到这个值之前的消息；

 ### :dash: LEO​ （Log End Offset）

- 日志末端位移：记录的该副本底层日志中下一条消息的位移值；

## 1.2 安装配置

:anchor: <font size=4 color=blue>JDK环境配置</font>

:anchor: <font size=4 color=blue>ZooKeeper安装</font>

- 下载并解压
- 修改日志数据目录并启动ZooKeeper
- 所有zookeeper节点都需要执行：zkServer.sh start

:anchor: <font size=4 color=blue>Kafka配置安装</font>

- 下载并解压（kafka有内置Zookeeper，可以使用Kafka内带的zk集群来启动，但是建议使用独立的zk集群）
- 修改kafka配置文件：/config/server.properties

  - kafka集群配置：broker.id和host.name每个节点都不相同；

  - 修改log.dirs的值；

  - 设置zookeeper的连接端口：zookeeper.connect；


:anchor: <font size=4 color=blue>启动Kafka</font>
...
:anchor: <font size=4 color=blue>Kafka命令行</font>
...
3. 查看已经创建的topic信息
   ...
   - --list：表示执行查询主题的列表的API

4. 生产者发送消息

   ```sh
   bin/kafka-console-producer.sh 
   --broker-list hadoop1:9092 
   --topic topic2
   ```
