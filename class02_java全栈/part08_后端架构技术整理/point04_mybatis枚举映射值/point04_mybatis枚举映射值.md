...
- application.yml
  ...
  mybatis:
    configuration:
      default-enum-type-handler: com.xxx.MyBatisEnumHandler
  ```


# ★ 使用描述

1. **新增**：IEnum类型的枚举对象作为持久化参数，存储到数据库的值是IEnum.getValue()的值
2. **更新**：IEnum类型的枚举对象作为持久化参数，在数据库的更新的值是IEnum.getValue()的值
3. **查询**：将IEnum对象作为参数，在SQL语句中的值是IEnum.getValue()的值
