- # 第一章 Security介绍
  ...
  - SpringSecurity概念
    - 认证：是指应用系统判断资源访问者的身份是否属于改应用
    - 会话：认证成功后便为该资源访问者创建了一个会话，在这个会话中保存有改访问者的信息，在当前会话的下次访问中使用该信息便不需要再次认证；
    - 授权：验证当前的访问者是否可以访问应用系统中的资源
  - 授权的数据模型
    - 授权概述：who 对 what 进行 how操作
    - 在应用系统中每个URL都唯一对应一组信息，也成为资源
    - RBAC：
      - 基于角色的访问控制：用户→角色→资源
      - 基于资源的访问控制：用户→资源
  - 分布式系统认证
    - 分布式系统：在早期的系统应用是一个单体应用，在应用中集成了所有模块的所有功能；在服务演化过程中单体应用中或根据功能模块拆分为多个单体应用，这些被拆分的单体应用被成为分布式系统；
    - 分布式系统特点
      - 分布式系统中每个应用都需要认证授权，所以分布式要提供统一认证的授权
      - 分布式系统中的系统支持第三方应用认证
    - 分布式系统认证：在分布式系统中每个应用服务都需要对访问者进行认证，面临的问题是在一个应用中认证后会话是报错在服务器端，无法同步到其他应用中，导致用户访问其他应用也需要认证，提高系统的使用复杂度；所以分布式认证系统的主要功能是只需要认证一次，认证成功后的认证信息在所有分布式系统之都有效
    - 分布式系统的解决方案：
      - session认证：分布式session管理，认证信息保存在服务端
      - token认证：token信息保存在服务端中

  1. OAuth2协议：
     - 是一个协议，开发的授权标准：标准的主要描述就是允许用户授权第三方应用访问他们存储在改服务中的信息，而不需要将用户名和密码提供给第三方应用；前提条件：存储数据的认证服务器需要提前知道第三方应用的存在；
  ...
  2. OAuth运行流程
     - 运行角色
       - Client：客户端、第三方客户端，需要通过资源所有者授权去请求资源服务器上的资源
       - 服务提供商（Provider）作用是提供令牌。谁提供令牌，谁就是服务提供商
         - Resource Owner：资源所有者，保存在资源服务器上的资源信息的拥有者
         - Authorization Server：认证服务器，验证客户端和资源所有者的认证信息，并发放token
       - Resource Server：资源服务器，保存资源所有者的信息
     - 运行流程
       - 用户访问第三方应用：想要获取微信照片
     ...
     - 环境搭建
       - 环境介绍
         - spring-security-oauth2：是对OAuth2的一种实现，与SpringSecurity相辅相成
         - spring-cloud体系中的分布式认证解决方案就是使用spring-security-oauth2，在SecurityOAuth2的服务提供方涵盖两个服务，即授权服务（认证服务）和资源服务（可以是独立的服务）
           - AuthorizationEndpoint：用于授权服务：用户认证请求，默认的url：/oauth、authorize
           - TokenEndpoint：用于访问令牌，默认url：/oauth/token
           - 资源服务：包含对资源的保护，对非法请求进行拦截，对请求中的token进行解析鉴权等等
         - OAuthAuthenticationProcessingFilter：SecurityOAuth中用来对请求给出的身份令牌解析鉴权
     - 在第二步有四种授权模式
       ...
       - 密码模式：
       - 客户端模式：
     - 授权码模式：http://localhost:8080/oauth/authorize?client_id=c1&response_type=code&scope=all&redirect_uri=http://www.baidu.com
     - 简化模式：http://localhost:8080/oauth/authorize?client_id=c1&response_type=token&scope=all&redirect_uri=http://www.baidu.com
     - 密码模式：http://localhost:8080/oauth/token?username=root&password=root&grant_type=password&scope=all&client_id=c1&client_secret=secret
     - 客户端模式：http://localhost:8080/oauth/token?grant_type=client_credentials&client_id=c1&client_secret=secret
     - http://localhost:8080/oauth/token?client_id=c1&client_secret=secret&grant_type=authorization_code&redirect_uri=http://www.baidu.com&code=
